...
void TwoBeams(MeshBlock *pmb, Coordinates *pco, NRRadiation *prad,
  ...
  Real cosx_hi = 0.0;                       // highest cosx(n_ang)
  int n_beam_ang = 0;                       // n_ang most parallel to x1 axis
        ...
        for (int ifr=0; ifr<nfreq; ++ifr) { // ith frequency bin
          for (int l=0; l<noct; ++l) {      // lth octant
            for (int n=0; n<ang_oct; ++n) { // nth octant angle
              ...
              if (gid == 0 && j == zone_index && l==octnum) {
                if (prad->mu(0,k,js-j,i,n_ang) > cosx_hi) {
                  cosx_hi = prad->mu(0,k,js-j,i,n_ang);
                  n_beam_ang = n_ang;
                }
              }
            }
          }
          if (gid == 0) ir(k,zone_index,is-i,n_beam_ang+ifr*nang) = 10.0;
        }
      }
    }
  }
