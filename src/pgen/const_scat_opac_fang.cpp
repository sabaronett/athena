...
Real dfloor;
Real Omega0;
Real x1min, npsi, crat, prat, T_unit, kappa_a, kappa_s, R, T;
} // namespace
      ...
      for (int j=js; j<=je; ++j) {
        for (int i=is; i<=ie; ++i) {
          for (int ifr=0; ifr < nfreq; ++ifr) {                         // opacities
            pnrrad->sigma_s(k,j,i,ifr) = phydro->u(IDN,k,j,i)*kappa_s;  // scattering
...
void RadInnerX1(MeshBlock *pmb, Coordinates *pco, NRRadiation *prad,
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=ngh; ++i) {
        for (int n=0; n<prad->nang; ++n) { // activate only forward angles
    ...
    for (int j=jl; j<=ju; ++j) {
      for (int i=il; i<=iu; ++i) {
        for (int ifr=0; ifr < prad->nfreq; ++ifr) {            // opacities
          prad->sigma_s(k,j,i,ifr) = prim(IDN,k,j,i)*kappa_s;  // scattering
